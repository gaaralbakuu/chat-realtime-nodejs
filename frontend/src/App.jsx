...
function App(props) {
  ...
  const {
    state: { username, isJoining, isJoined },
  ...
  useEffect(() => {
    ...
    if (isJoining) {
      ...
      socket.on("connect", function () {
        dispatch(setStateJoined(true));
        dispatch(setStateJoining(false));
        socket.emit("set-username", username);
      });

      socket.on("status-users", function (data) {
        dispatch(setUsers(data));
      });

      socket.on("status-rooms", function (data) {
      ...
      socket.on("response", function ({ type, message }) {
        console.log(message)
        toast(message, { type });
      });
